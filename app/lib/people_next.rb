class PeopleNext
  def self.my_member #ボーナス対象人数（月。クオーター）
    ...
    @sub.each do |z|
      ...
      if row.empty?
        member["mb_#{@last_month}"] = 0
      elsif row.length == 1
        member["mb_#{@last_month}"] = 1
      elsif row.length == 2
        member["mb_#{@last_month}"] = len1 + len2
      elsif row.length == 3
        member["mb_#{@last_month}"] = len1 + len2 + len3
      elsif row.length == 4
        member["mb_#{@last_month}"] = len1 + len2 + len3 + len4
      elsif row.length == 5
        member["mb_#{@last_month}"] = len1 + len2 + len3 + len4 + len5
      elsif row.length == 6
        member["mb_#{@last_month}"] = len1 + len2 + len3 + len4 + len5 + len6
      else
        member["mb_#{@last_month}"] = len1 + len2 + len3 + len4 + len5 + len6 + len7
      end
      
      if (2 <= @month && @month <= 4)
        member.mbc_1 = member.mb_1 + member.mb_2 + member.mb_3       #1クオーターの人数集計
      elsif (5 <= @month && @month <= 7)
        member.mbc_2 = member.mb_4 + member.mb_5 + member.mb_6       #2クオーターの人数集計
      elsif (8 <= @month && @month <= 10)
